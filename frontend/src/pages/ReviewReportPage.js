...
const ReviewReportPage = () => {
  ...
  useEffect(() => {
    const fetchReportData = async () => {
      try {
        const response = await axios.get(
          "http://localhost:5000/api/reviews/report"
        );
    ...
    const fetchMonthlyReport = async () => {
      try {
        const response = await axios.get(
          "http://localhost:5000/api/reviews/monthly-report"
        );
    ...
    <div className="report-container">
      ...
      <div className="report-summary">
        <div className="report-card">
          <h4> Total Reviews </h4> <p> {totalReviews} </p>{" "}
        </div>{" "}
        <div className="report-card">
          <h4> Average Rating </h4> <p> {averageRating.toFixed(1)}⭐ </p>{" "}
        </div>{" "}
      </div>
      ...
      <div className="table-container">
        ...
        <div className="table-wrapper">
          <table>
            <thead>
              <tr>
                <th> Month </th> <th> Total Reviews </th>{" "}
